...
The loader reads every sheet in the workbook and writes each one to a
MySQL table named after the sheet (lowercased, spaces → underscores).
...
Open the `.pbix` file and refresh. The model will pull fresh data
through the ODBC connection defined in the file.
...
| `Products` | `products` |
| `Customers` | `customers` |
| `security.csv` (separate file) | `security` |

### Step 2 — Raw tables → Star schema (`scripts/sql/*.sql`)

Six SQL files build the presentation layer, followed by a validation
script. Each is idempotent — safe to re-run without side effects.
...
(`order_date`). Additional date columns (`order_confirmation_date`,
...
| 6 | `security` row count matches source | Identical |
